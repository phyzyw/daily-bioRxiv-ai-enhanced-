...
### [1] [Classic machine learning on top of multiple position weight matrices improves genomic prediction of transcription factor binding sites](https://www.biorxiv.org/content/10.64898/2026.05.12.724515)
*Kravchenko, P., Vorontsov, I. E., Makeev, V. J., Kulakovskiy, I. V., Penzar, D. D.*

Main category: machine learning

TL;DR: ArChIPelago：combining PWMs for improved transcription factor binding site prediction


<details>
  <summary>Details</summary>
Motivation: combining multiple PWMs to improve prediction accuracy

Method: machine learning techniques, linear regression to decision trees

Result: outperforms individual PWMs and sparse local inhomogeneous mixture models

Conclusion: reliable cross-species predictions possible
...
### [2] [Explainable prediction and simulation of complex system dynamics through networks of manifolds](https://www.biorxiv.org/content/10.64898/2026.05.12.724527)
*Park, J., Smith, C., Tseng, S. Y., Guidera, J., Semenov, A. V., Smirnov, S., Frank, L. M., Pao, G. M.*

Main category: machine learning

TL;DR: GMNs：一种机器学习框架，用于捕捉多变量、多尺度的非线性交互。


<details>
  <summary>Details</summary>
Motivation: 复杂系统难以表示，因为它们涉及多变量、多尺度和非线性交互。

Method: GMNs由网络节点组成，每个节点是低维度数据驱动的状态空间曲面，使用生成函数模拟多尺度动态。

Result: GMNs可以学习复杂系统的完整动态，且可以生成短期和长期的混沌动态。

Conclusion: GMNs是universal approximators，可以用于捕捉多变量、多尺度的非线性交互。
...
### [3] [Pathogen-specific host responses define distinct pneumonia endotypes in the human lung](https://www.biorxiv.org/content/10.64898/2026.05.12.724509)
...
TL;DR:  pneumonia diagnosis and treatment rely on syndromic definitions, this study uses machine learning to identify pathogen-specific host responses for individualized therapy.


<details>
  <summary>Details</summary>
Motivation: pneumonia diagnosis and treatment lag behind other major conditions, seeking to identify distinct pneumonia endotypes linked to clinical features and outcomes.

Method: prospective cohort study, 690 patients, 792 BAL fluid samples, flow cytometry, single-cell RNA-sequencing, machine learning models.

Result: pathogen-specific host responses associated with changes in alveolar cell abundance and clinical features, suggesting individualized therapy.

Conclusion: therapeutic strategies for pneumonia should be individualized to specific host-pathogen interactions.
...
### [4] [Pathway-Centric Integration of CRISPR Fitness with Molecular Features Draws Cancer State Maps](https://www.biorxiv.org/content/10.64898/2026.05.12.724547)
*Hagiwara, T., Ito, K. K., Kuroki, K., Niikura, R., Hirota, M., Kawai, K., Uga, H., Iwakiri, J., Terai, G., Asai, K., Goyama, S., Shikata, S., Kanai, A., Suzuki, Y., Shimada, K., Fukuyama, M., Hata, S., Yamamoto, S., Chinen, T., Kitagawa, D.*

Main category: machine learning

TL;DR: StateMap framework integrates gene expression and CRISPR knockout data to map pathway activity and identify therapeutic vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Mapping heterogeneous pathway activity in cancer cells is challenging due to limitations of transcriptomic scores and CRISPR knockout data.

Method: StateMap selects features by co-dependency and mutual information, then projects cell lines into a low-dimensional space.

Result: Resolved five functional states of the Hippo pathway, identified state-selective therapeutic vulnerabilities, and predicted sensitivity to Cilengitide.

Conclusion: StateMap enables pathway-specific state mapping and discovery of state-selective therapeutic vulnerabilities.

Abstract: Cancer cells display heterogeneous pathway activity that shapes therapeutic vulnerability, but mapping it remains challenging. Transcriptomic scores do not directly measure functional activity, and CRISPR knockout data alone lack molecular interpretability. We introduce StateMap, a pathway-centric framework integrating gene expression and genome-wide CRISPR knockout fitness data from the Cancer Dependency Map. For a given pathway, StateMap selects features by co-dependency and mutual information, then projects cell lines into a low-dimensional space reflecting pathway activity and molecular state. Applied to the Hippo pathway, it resolved five functional states refining the YAP-on/YAP-off dichotomy. Notably, the 'Hippo-strong' state showed selective dependence on integrin V{beta}5; ITGAV depletion triggered Hippo-dependent cell aggregation and G1 arrest via enhanced cell-cell adhesion. Machine learning transfer to TCGA identified a matching subtype with poor prognosis, nominated NNMT as a biomarker, and predicted sensitivity to the V inhibitor Cilengitide. StateMap enables pathway-specific state mapping and discovery of state-selective therapeutic vulnerabilities.

</details>


### [5] [Plasmatic immune extracellular vesicle profiles identify prodromal and early stages of Parkinson's disease](https://www.biorxiv.org/content/10.64898/2026.05.12.724498)
...
TL;DR: EVs在神经退行性疾病中的诊断价值


<details>
  <summary>Details</summary>
Motivation: 明确疾病和阶段特定的EV表型

Method: 纳米粒子跟踪分析、流式细胞术和机器学习算法

Result: 发现了不同疾病组的EV表型和诊断标记

Conclusion: EV immuno-phenotyping在prodromal iRBD检测中具有最高的诊断价值
...
### [8] [Identification of non-covalent inhibitors for the atypical peroxiredoxin PRDX5 as a therapeutic strategy in malignant pleural mesothelioma](https://www.biorxiv.org/content/10.64898/2026.05.13.724787)
...
TL;DR: MPM中PRDX5为新型治疗靶点，开发红氧调节治疗方法


<details>
  <summary>Details</summary>
Motivation: MPM治疗有限，寻找红氧调节靶点

Method: MOSAIC atlas、机器学习、虚拟筛选、高通道验证

Result: PRDX5为MPM特异靶点，发现36个非共价抑制剂

Conclusion: PRDX5为MPM治疗新希望
...
### [9] [Segmentation and profile-based classification of movement strategies from animal tracking data](https://www.biorxiv.org/content/10.64898/2026.05.13.724011)
*Kadlec, I., Bartak, V., Selimovic, A., Kutal, M., Dula, M., Stier, N., Meissner-Hylanova, V., Peskova, L. B., Sladecek, M., Vorel, A., Signer, J.*

Main category: machine learning

TL;DR: 基于GPS追踪数据的动物运动策略分类方法


<details>
  <summary>Details</summary>
Motivation: 解决现有方法的限制，提供可靠的分类结果和不确定性估计

Method: 基于NSD时间序列的断点检测、行为-profile比较和Bootstrap resampling

Result: 91.1%的准确率，5-10个训练段落足以确保可靠分类

Conclusion: 提供了一个可访问、可解释的分类框架，适用于任何跟踪物种

Abstract: 1. Classifying animal movement strategies from GPS tracking data is essential for understanding space use, population dynamics and conservation planning. However, existing approaches either require strong parametric assumptions about trajectory shape, large labelled datasets (i.e. expert-annotated) for machine learning, or lack formal uncertainty quantification. These limitations create barriers for researchers working with novel species or limited sample sizes. 2. We present a profile-based classification framework consisting of three steps. First, trajectories are segmented using breakpoint detection applied to Net Squared Displacement (NSD) time series. Movement metrics are then extracted from each segment and classified by comparing them to empirically derived behavioural profiles via Z-score distances transformed to softmax probabilities. Bootstrap resampling quantifies uncertainty in the resulting classifications from both training and test data. We validated the framework through simulation experiments and applied it to GPS tracking data from two ecologically contrasting species: gray wolf (Canis lupus; 43 individuals) and northern lapwing (Vanellus vanellus; 15 individuals). 3. Simulations showed that 5-10 training segments per movement strategy suffice for reliable classification, with overall accuracy of 91.1% across residential, floating and dispersal strategies. Segment duration of 30-60 days was required for confident discrimination of residential and floating behaviour. For wolves, the framework clearly distinguished residency, floating or dispersal (91.2% of segments classified with >50% probability). For lapwings, migration was identified with high confidence, while residential-floating discrimination reflected genuine ecological ambiguity confirmed by domain experts, with bootstrap confidence intervals transparently flagging uncertain cases. 4. The profile-based framework provides an accessible, interpretable alternative to parametric NSD fitting and a machine learning approach, requiring modest training data while delivering probabilistic classifications with honest uncertainty estimates. An R package (moveprofile) implementing the complete workflow is freely available. The framework is applicable to any tracked species where distinct movement strategies can be identified by experts' knowledge.
