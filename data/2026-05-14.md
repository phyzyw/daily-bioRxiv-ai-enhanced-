...
### [1] [Generative machine learning unlocks the first proteome-wide image of human cells](https://www.biorxiv.org/content/10.64898/2026.03.31.715748)
...
TL;DR: ProtiCelli：一个深度生成模型，模拟细胞中12,800种蛋白质的微观图像


<details>
  <summary>Details</summary>
Motivation: 解决当前成像技术只能同时 visualize数十种蛋白质的限制

Method: 基于1.23万张Human Protein Atlas图像训练的深度生成模型

Result: 重建准确率高、文本忠实度高、泛化能力强

Conclusion: ProtiCelli为空间虚拟细胞建模奠定基础，推动了空间蛋白质学从cataloging蛋白质到模拟完整细胞系统的转变
...
### [4] [Morphological fingerprints enable machine learning based inference of neuroblastoma cell states without transcriptomics](https://www.biorxiv.org/content/10.64898/2026.05.12.724731)
*Zamloot, V., Pan, Y., Park, J.*

Main category: machine learning

TL;DR: 基于细胞形态学的机器学习框架用于实时监测癌细胞状态


<details>
  <summary>Details</summary>
Motivation: 当前基于转录组学的方法限制了癌细胞状态的可扩展性和实时监测

Method: 使用神经母细胞瘤模型建立了机器学习-形态学 profiling 框架

Result: 细胞状态与转录组学-profiles在单细胞分辨率上高度一致

Conclusion: 细胞形态学作为可扩展和非破坏性的癌细胞状态读出

Abstract: Inference of cancer cell states is essential for understanding oncogenic mechanisms and predicting clinical outcomes, yet current reliance on transcriptomic profiling limits scalability and real-time monitoring. Here, we show that cell morphology provides a low-dimensional, observable representation of cellular identity and its dynamics. Using neuroblastoma (NB) as a model system, we establish a machine learning- morphology profiling framework that infers adrenergic (ADRN) and mesenchymal (MES) cell states directly from high-dimensional morphological fingerprints without reliance on transcriptomic measurements. By benchmarking against single-cell RNA sequencing (scRNA-seq), we demonstrate that morphology-defined states closely align with transcriptomic profiles at single-cell resolution. We further show that cell state transitions are represented as continuous trajectories within a morphology-defined state space. Perturbations targeting distinct regulatory layers, including ROCK signaling and epigenetic regulation via EZH2, drive convergent trajectories along a shared phenotypic axis. Together, these results establish cell morphology as a scalable and non-destructive readout of cell state with machine learning providing a unified framework for high-throughput phenotyping and real-time tracking of cancer cell state plasticity.

</details>


### [5] [Integration of immunomonitoring assays with PET/CT in TB patients identifies on-treatment biomarkers](https://www.biorxiv.org/content/10.64898/2026.05.07.723467)
...
### [8] [Bayesian-Steered Structure Prediction of Mechanical Biomolecules Using Twisted Diffusion](https://www.biorxiv.org/content/10.64898/2026.05.11.724187)
*Klaus, C., Sotomayor, M.*

Main category: molecular dynamics

TL;DR: 使用生成式人工智能扩散模型预测蛋白质结构的非平衡态


<details>
  <summary>Details</summary>
Motivation: 探索实验数据中未报告的功能相关态

Method: 使用Twisted diffusion sampler在Boltz-2中采样条件分布

Result: 重现DNA、titin、protocadherin-15和MscL离子通道的非平衡态

Conclusion: 有望帮助样本多种生物大分子系统的非平衡态
